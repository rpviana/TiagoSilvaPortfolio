...
const Events = () => {
  ...
  const { data: events = [], isLoading, error } = useQuery<EventWithTranslations[]>({
    queryKey: ['/api/events'],
    queryFn: async () => {
      ...
      // Update isPast based on current date
      const now = new Date();
      return data.map((event: EventWithTranslations) => ({
        ...event,
      ...
      <div className="pt-24">
        <section className="py-16 md:py-24 bg-gray-50">
          <div className="container mx-auto px-4 text-center">
            <p className="text-red-600">{t('events.errorLoading')}</p>
          </div>
        </section>
      </div>
    );
